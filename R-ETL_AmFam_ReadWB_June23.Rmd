---
title: "AMFam-CA LoB-ReadWebpage ETL, Author: Chinmay Patane"
...
#Read the requirements from the requirement page directly (either from requirement page, webpage, pdf, word exported from that page)
...
```{r}
#Get the requirement page .docx file into the variable, and read its content
Get_Document = read_docx("August 2023.docx") #In case of reading direct web Page, web page link with credentials will replace this command
content <- docx_summary(Get_Document)

#Get the index number of the target text '5.1. Rate Table Changes' and 5.2. Referenced Documents
X <- content %>% filter(text == '5.1. Rate Table Changes') %>% pull(doc_index)
X <- as.numeric(X)
Y <- content %>% filter(text == '5.2. Referenced Documents') %>% pull(doc_index)
Y <- as.numeric(Y)

#Filter the data with these indexes and get the table out where requirements for CA LoB are mentioned
content <- content %>% filter(doc_index >= X  &  doc_index <= Y)
table_cells <- content %>% filter(content_type == "table cell")

#Get the table as Get_Requirements 
...
Get_Requirements <- bind_cols(splits)
# get table headers
cols <- table_cells %>% filter(is_header)
names(Get_Requirements) <- cols$text


#Modify the columns of the Get_Requirements
...
# Convert NB_Effective_Date and RN_Effective_Date mm/dd/yyyy format
Get_Requirements$NB_Effective_Date <- mdy(Get_Requirements$NB_Effective_Date)
Get_Requirements$RN_Effective_Date <- mdy(Get_Requirements$RN_Effective_Date)
...
for (i in 1:length(Factor_Name)) {
  ...
  #Start exporting the data in .xlsx format after transforming 
...
# Export the report as well in .xlsx to verify all went well
